...
class AnalyticsManager constructor(
    private val hiAnalyticsImpl: HiAnalyticsImpl
) : IAnalyticsEvents {

    override fun userLogin(userCredential: Credential) {
        hiAnalyticsImpl.userLogin(userCredential)
    }

    override fun userLogout() {
        hiAnalyticsImpl.userLogout()
    }

    override fun enterScreen(screen: String) {
        hiAnalyticsImpl.enterScreen(screen)
    }

    override fun permissionNotAllowedAppClosed() {
        hiAnalyticsImpl.permissionNotAllowedAppClosed()
    }

    override fun landmarkRecognized(landmarkName: String) {
        hiAnalyticsImpl.landmarkRecognized(landmarkName)
    }

    override fun userPreparedImage(isTakeFromCamera: Boolean) {
        hiAnalyticsImpl.userPreparedImage(isTakeFromCamera)
    }

    override fun clickedPanoramicImage(landmarkName: String) {
        hiAnalyticsImpl.clickedPanoramicImage(landmarkName)
    }

    override fun clickedPanoramicVideo(landmarkName: String) {
        hiAnalyticsImpl.clickedPanoramicVideo(landmarkName)
    }
}
